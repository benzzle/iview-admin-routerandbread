# iview-admin-routerandbread
基于iview-admin的后台管理系统，不用开发者处理路由和面包屑的深度展示，把原来的项目删减和增加一些便捷的操作

优化点：1.面包屑的可以点击，
        有些页面路由是会带一定的参数进行请求数据的，这些参数可能来自于上个页面。当页面刷新的时候如果不是数据没有放在页面路由中那么就会在刷新页面的时候拿不到数据。
<p align="center">
